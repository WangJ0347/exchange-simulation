...
func (os *OrderSide) Append(o *Order) *list.Element {
	price := o.Price()
	strPrice := o.PriceKey() // Use cached key to avoid allocation
...
func (os *OrderSide) Remove(e *list.Element) *Order {
	...
	strPrice := e.Value.(*Order).PriceKey() // Use cached key to avoid allocation
...
// GetOrderQueue returns the OrderQueue for a specific price, or nil if it doesn't exist
func (os *OrderSide) GetOrderQueue(price decimal.Decimal) *OrderQueue {
	strPrice := price.String() // Note: Can't use cached key here since we only have price
	priceQueue, ok := os.prices[strPrice]
	if !ok {
		return nil
	}
