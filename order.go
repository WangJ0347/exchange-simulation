...
type Order struct {
	...
	priceKey  string // Cached price.String() for map lookups (performance optimization)
}
...
func NewOrder(orderID string, side Side, quantity, price decimal.Decimal, timestamp time.Time) *Order {
	return &Order{
		...
		quantity:  quantity,
		price:     price,
		priceKey:  price.String(), // Cache for performance
		timestamp: timestamp,
	}
}
...
// PriceKey returns cached price string for map lookups (performance optimization)
...
func (o *Order) UnmarshalJSON(data []byte) error {
	...
	o.price = obj.Price
	o.priceKey = obj.Price.String() // Cache for performance
	return nil
}
